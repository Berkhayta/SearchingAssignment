...
int binarySearchFirst(std::vector<T> vec, T target) {
    int index = binarySearch(vec, target);  // İlk olarak normal binary search ile arama yap
    if (index == -1) {
        return -1;  // Eğer bulunmadıysa -1 döndür
    }

    // İlk konumu bulmak için geriye doğru ilerle
...
int binarySearchFirst(std::array<T, N> arr, T target) {
    int index = binarySearch(arr, target);  // İlk olarak normal binary search ile arama yap
    if (index == -1) {
        return -1;  // Eğer bulunmadıysa -1 döndür
    }
